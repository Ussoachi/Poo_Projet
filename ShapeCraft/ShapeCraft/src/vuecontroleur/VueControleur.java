...
public class VueControleur extends JFrame implements Observer {
    ...
    private void mettreAJourPanneauObjectif() {
        ZoneLivraison zl = jeu.getZoneLivraison();

        // Numéro de l'objectif (1-indexé pour l'affichage)
        int numero = jeu.getIndexObjectif() + 1;
        labelObjectifNumero.setText("Objectif " + numero + "/3");

        // Progression
        labelObjectifProgression.setText(
                zl.getQuantiteRecue() + " / " + zl.getQuantiteObjectif()
        );

        // Forme attendue
        panneauFormeObjectif.setShape(zl.getObjectifActuel());
        panneauFormeObjectif.repaint();
    }
}